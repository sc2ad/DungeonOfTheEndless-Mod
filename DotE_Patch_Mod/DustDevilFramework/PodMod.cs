...
namespace DustDevilFramework
{
    public abstract class PodMod : ScadMod
    {
        ...
        public void Initialize()
        {
            ...
            config = GetName() + "_config.txt";
            default_config = "# Modify this file to change various settings of the " + GetName() + " mod for DotE.\n" + default_config;
            base.Initialize();

            // Setup default values for config
            Values.Add("AssumedPod", GetAnimationPod());
            Values.Add("DungeonPod", GetDungeonPod());

            ReadConfig();
            if (Convert.ToBoolean(Values["Enabled"]))
            {
                Log("Attempting to create Localization changes!");
                this.CreateLocalizationChanges();
                //Log("Attempting to copy sounds!");
            }
            Log("Initialized!");
        }
        ...
        public void Load()
        {
            ...
            else
            {
                ...
                this.RemoveLocalizationChanges();
                //Log("Attempting to remove sounds!");
            }
        }
        ...
        private System.Collections.IEnumerator DungeonGenerator2_GenerateDungeonUsingSeedCoroutine(On.DungeonGenerator2.orig_GenerateDungeonUsingSeedCoroutine orig, DungeonGenerator2 self, int level, int randomSeed, Amplitude.StaticString shipName)
        {
            if (shipName == GetName())
            {
                ...
                yield return typeof(DungeonGenerator2).GetMethod("GenerateDungeonCoroutine", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).Invoke(self, new object[] { level, new Amplitude.StaticString(Values["DungeonPod"]) });
        ...
        private void GameSelectionPanel_LaunchSelectedShipConfirmAnimation(On.GameSelectionPanel.orig_LaunchSelectedShipConfirmAnimation orig, GameSelectionPanel self)
        {
            ...
            if (temp == GetName())
            {
                d.Set<Amplitude.StaticString>("selectedShipName", Values["AssumedPod"]);
            }
            orig(self);
        ...
        private SpriteAnimationRuntime2 GameSelectionPanel_GetShipAnimation(On.GameSelectionPanel.orig_GetShipAnimation orig, GameSelectionPanel self, Amplitude.StaticString shipName)
        {
            ...
            if (shipName == GetName())
            {
                return orig(self, Values["AssumedPod"]);
            }
